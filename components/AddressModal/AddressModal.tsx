...
const AddressModal = ({ 
    ...
    const getPanelValue = async (searchText: string) => {
        ...
        const data = response.map((res: { full_address: string, title_address: string }) => ({ value: res.full_address, title: res.title_address }));
        setOptions(data);
    }

    return (
        <Modal title="Delivery Address" open={isOpen} onOk={handleOk} onCancel={handleCancel} okButtonProps={{ style: { backgroundColor: "var(--orange-4)" } }} cancelButtonProps={{ style: { backgroundColor: "transparent" } }}>
            <AutoComplete 
